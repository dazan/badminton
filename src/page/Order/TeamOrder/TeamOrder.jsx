...
import { SearchInput, Table, Notify, Button, Sweetalert, Select, DatePicker } from 'zent';
import { getAllTeamOrder, deleteTeamOrderById } from '../../../api/teamOrder.js';
...
//                 <Button data-id={ data.id } data-first={ pos.row === 0 ? 1 : 0 } className="delete-team-order-btn" type="danger" size="small">删除</Button>
//             </div>
//         );
//     }
// }
...
class TeamOrder extends React.Component {
    ...
    componentWillMount(){
        ...
        this.getAllOrder(function(){
            const allDeleteBtn = document.querySelectorAll('.delete-team-order-btn'); // 为所有删除按钮绑定click事件
            allDeleteBtn.forEach(item => {
                item.addEventListener('click', that.deleteOrderHandler, false);
            });
        });
    }
    componentWillUnmount() {
        ...
        const that = this;
        const allDeleteBtn = document.querySelectorAll('.delete-team-order-btn'); 
        allDeleteBtn.forEach(item => {
            item.removeEventListener('click', that.deleteOrderHandler, false);
        });
    }
    // 包装一层是为了在组件摧毁时，能解绑事件
        ...
        if(isTheFirstInLastPage === true){ // 如果是最后一页剩下的最后一条，那么请求“前一页的数据”
            ...
            deleteTeamOrderById({
                ...
                that.setState({
                    ...
                    that.getAllOrder(function(){
                    const allDeleteBtn = document.querySelectorAll('.delete-team-order-btn'); // 需要重新绑定删除按钮的click事件
                    allDeleteBtn.forEach(item => {
                        item.removeEventListener('click', that.deleteOrderHandler, false);
                        item.addEventListener('click', that.deleteOrderHandler, false);
                    });
            ...
            if(res.data.code === 0 && that._isMounted === true){
                ...
                const all = list.map((item, index) => {
                    return {
                        ...
                        groupCourseName: item.items[0].info && item.items[0].info.groupCourseName && item.items[0].info.groupCourseName,
    ...
    searchHandler(){
        ...
        this.setState({
            ...
            reqCourseNameKey: this.state.courseNameKey.trim(),
            ...
            <div className="team-order">
                <div className="team-order-filter">
                        <div className="top">
                            ...
                            <SearchInput
                                placeholder="请输入课程名"
                                value={ this.state.courseNameKey }
                                onChange={ (e) => this.handleChange('courseNameKey', e) }
                                className="top-input"
                            />
                            <SearchInput
                                placeholder="请输入用户名"
